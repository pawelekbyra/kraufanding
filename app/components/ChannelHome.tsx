...
export default function ChannelHome({ mainVideo, allVideos, currentVideoId, userProfile }: ChannelHomeProps) {
  ...
  const playlistItems = sortedVideos.reduce((acc: any[], video, i) => {
      ...
      acc.push(
          <div
            ...
            className={cn(
              ...
              isCurrent ? "bg-gold/5 border-gold/20" : "hover:bg-charcoal/50 hover:border-gold/10"
            )}
          >
            <Link
            ...
            />
            <div className="w-[160px] h-[90px] shrink-0 overflow-hidden rounded-none bg-black relative z-10 group/thumb border border-gold/10 group-hover:border-gold/30 transition-colors">
              <Link
                href={video.id === mainVideo.id ? "/" : `/?v=${video.id}`}
                scroll={false}
              ...
              {video.duration && (
                <div className="absolute bottom-1 right-1 bg-obsidian/90 text-gold text-[10px] font-mono font-bold px-1.5 py-0.5 rounded-none border border-gold/20 z-30 pointer-events-none tracking-tighter">
                   {video.duration}
                </div>
              )}
            </div>
            <div className="flex-1 min-w-0 flex flex-col justify-center gap-1 z-10">
              ...
              >
                <h4 className={cn(
                  ...
                  isCurrent ? "text-gold" : "text-bone"
                )}>
                   {video.title}
                </h4>
              </Link>
              <div className="text-[12px] text-bone/50 flex flex-col mt-0.5 font-mono">
                 <Link
                   href={video.creator?.slug ? `/channel/${video.creator.slug}` : "#"}
                   className="hover:text-gold transition-colors hover:underline w-fit relative z-20"
                 >
              ...
              {mounted && (
                hasAccess ? (
                  <span className="text-[9px] font-black uppercase tracking-[0.2em] text-gold mt-1 drop-shadow-[0_0_8px_rgba(197,160,89,0.3)]">{t.available}</span>
                ) : video.tier === 'LOGGED_IN' ? (
                  <span className="text-[9px] font-black uppercase tracking-[0.2em] text-blue-400 mt-1">{language === 'pl' ? 'Pokaż Dowód' : 'Identify'}</span>
                ) : (
                  <span className="text-[9px] font-black uppercase tracking-[0.2em] text-crimson mt-1">{language === 'pl' ? 'Zasoby Zastrzeżone' : 'Restricted Access'}</span>
                )
              )}
            </div>
          </div>
      ...
      if (i === 1) {
        acc.push(
          <div key="donate" className="py-4 border-y border-gold/10 bg-gold/[0.02]">
              <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-gold/40 mb-3 px-2 font-mono">{t.donate}</h3>
              <div className="px-1">
                <VideoPlaylist
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-12 gap-8">
          <div className="col-span-12 lg:col-span-8 space-y-6">
            ...
            <div className="lg:hidden flex border-b border-gold/10">
               <button
                 onClick={() => setActiveTab('comments')}
                 className={cn(
                   "flex-1 py-4 text-xs font-black uppercase tracking-[0.2em] transition-all border-b-2 font-mono",
                   activeTab === 'comments' ? "border-gold text-gold" : "border-transparent text-bone/30"
                 )}
               >
                 {t.comments}
               </button>
               <button
                 ...
                 className={cn(
                   "flex-1 py-4 text-xs font-black uppercase tracking-[0.2em] transition-all border-b-2 font-mono",
                   activeTab === 'videos' ? "border-gold text-gold" : "border-transparent text-bone/30"
                 )}
               >
            ...
            <div className="lg:hidden">
               {activeTab === 'comments' ? (
                 <div className="bg-charcoal/30 p-4 border border-gold/5">
                   <EmbeddedComments
                 ...
                 <div className="space-y-4">
                    {playlistItems}
                    {searchQuery && (
                      <div className="pt-6 border-t border-gold/10 mt-4">
                        <Link
                          href="/"
            ...
            <div className="hidden lg:block">
               <div className="bg-charcoal/30 p-6 border border-gold/5">
                 <EmbeddedComments
                   videoId={selectedVideo.id}
                   userProfile={userProfile}
                 />
          ...
          <aside className="hidden lg:block lg:col-span-4 space-y-4 sticky top-24 h-fit">
            <div className="flex justify-between items-end border-b border-gold/10 pb-2 mb-4">
              <div className="flex items-center gap-3">
                <h3 className="text-[12px] font-bold uppercase tracking-[0.3em] text-gold font-mono">
              ...
              <div className="flex gap-6 mb-[-2px] font-mono">
                <button
                  ...
                  className={cn(
                    ...
                    language === 'pl' ? "text-gold border-b border-gold pb-0.5" : "text-bone/20 hover:text-bone/40"
                  )}
                >
                  PL
                </button>
                <button
                  ...
                  className={cn(
                    "text-[10px] font-black tracking-[0.2em] uppercase transition-all",
                    language === 'en' ? "text-gold border-b border-gold pb-0.5" : "text-bone/20 hover:text-bone/40"
                  )}
                >
                  EN
                </button>
            ...
            {playlistItems.length > 0 ? (
                <div className="space-y-4">
                  ...
                  {searchQuery && (
                    <div className="pt-6 border-t border-gold/10 mt-6">
                      <Link
                        href="/"
                        className="btn btn-block brutalist-button italic"
                      >
                ...
                <div className="py-20 text-center border border-gold/10 bg-charcoal/30">
                    <p className="font-serif italic text-base text-bone/40 mb-8">
                        {language === 'pl' ? 'Brak zeznań dla tej kwerendy.' : 'No evidence found for this query.'}
                    </p>
                    <Link
                      href="/"
                      className="btn brutalist-button px-8 italic"
                    >
