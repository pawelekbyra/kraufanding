...
export default async function Paywall({
  ...
  // 1. FREE (Authenticated, but status unknown or Free)
  // In a real app, we'd check the Prisma DB here.
  // We'll simulate the DB check.
  const user = await prisma.user.findUnique({
    where: { clerkUserId: userId },
    include: { projectAccess: { where: { projectId } } }
  }).catch(() => null);

  // If user doesn't exist in DB, create them as FREE
  if (!user) {
    // This part would normally happen in a webhook, but we handle it here for demo
    // await prisma.user.create({ data: { clerkUserId: userId, email: (await currentUser())?.emailAddresses[0].emailAddress || "" } });
  }

  const userTierLevel = user?.tier || UserTier.FREE;
  const tiers = [UserTier.GUEST, UserTier.FREE, UserTier.OBSERVER, UserTier.WITNESS, UserTier.INSIDER, UserTier.ARCHITECT];
  const userTierIndex = tiers.indexOf(userTierLevel);
  const minTierIndex = tiers.indexOf(minTier);

  const hasAccess = userTierIndex >= minTierIndex || (user?.projectAccess.length || 0) > 0;
    ...
    return (
      <div className="space-y-12">
        ...
        <div className="p-8 bg-primary/5 border border-primary/20 rounded-2xl animate-in fade-in duration-700">
          ...
          <div className="aspect-video bg-neutral/10 rounded-xl overflow-hidden mb-4">
            <img src="https://picsum.photos/seed/sample/800/450" alt="Free Sample" className="object-cover w-full h-full opacity-90" />
          </div>
        </div>
        ...
        <div className="p-12 bg-white border-4 border-double border-neutral/10 rounded-3xl flex flex-col items-center text-center shadow-2xl relative overflow-hidden">
          ...
          <h3 className="text-3xl font-black mb-4 uppercase tracking-tighter">Become an Observer</h3>
